...
public class Malevich implements Listener {
    ...
    public Malevich(JavaPlugin plugin) {
        ...
        // 1) Every 5 ticks: fill empty slots only for players in allowed dims who have a malevich
        new BukkitRunnable() {
            @Override
            public void run() {
                ...
                for (Player player : Bukkit.getOnlinePlayers()) {
                    ...
                    if (isExcludedDimension(world)) continue;

                    PlayerInventory inv = player.getInventory();
                    if (!hasMalevichInInventory(inv)) continue;
        ...
        // 2) Every second: process malevich items on the ground, except in excluded dims
        new BukkitRunnable() {
            @Override
            public void run() {
                ...
                for (Item male : males) {
                    ...
                    for (Entity near : male.getNearbyEntities(3, 3, 3)) {
                        if (!(near instanceof Item)) continue;
                        Item other = (Item) near;
                        if (isMalevich(other)) continue;
    ...
    private void fillInventory(PlayerInventory inv, LootTable table, LootContext ctx) {
        ...
        if (inv.getHelmet() == null)     populateAndSet(inv::setHelmet,      table, ctx);
        if (inv.getChestplate() == null) populateAndSet(inv::setChestplate,  table, ctx);
        if (inv.getLeggings() == null)   populateAndSet(inv::setLeggings,    table, ctx);
        if (inv.getBoots() == null)      populateAndSet(inv::setBoots,       table, ctx);
    ...
    private boolean hasMalevichInInventory(PlayerInventory inv) {
        ...
        if (contents != null) {
            for (ItemStack stack : contents) {
                if (isMalevichStack(stack)) return true;
            }
        }
        if (isMalevichStack(inv.getItemInOffHand()))   return true;
        if (isMalevichStack(inv.getHelmet()))         return true;
        if (isMalevichStack(inv.getChestplate()))     return true;
        if (isMalevichStack(inv.getLeggings()))       return true;
    ...
    private boolean isMalevichStack(ItemStack stack) {
        ...
        if (!p.has(itemKey, PersistentDataType.TAG_CONTAINER)) return false;
    ...
    /**
     * Returns true if the world's dimension key is "minecraft:nexus" or "minecraft:imprinted".
     */
