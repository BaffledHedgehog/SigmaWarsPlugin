...
/**
 * Clones the entire chunk around the command origin (including /execute at) to
 * Nexus.
 */
    ...
    public void execute(CommandSourceStack src, String[] args) {
        ...
        Future<?> future = completionService.submit(() -> {
            try {
                ...
                BlockVector3 max = BlockVector3.at((cx << 4) + SIZE - 1, srcWorld.getMaxHeight() - 1,
                        (cz << 4) + SIZE - 1);
                    ...
                    public void run() {
                        try {
                            ...
                            for (var e : srcC.getEntities()) {
                                if (e instanceof Player || e.getType() == EntityType.MARKER)
                                    continue;
                ...
                synchronized (jsonLock) {
                    ...
                    Type listType = new TypeToken<List<Map<String, Number>>>() {
                    }.getType();
